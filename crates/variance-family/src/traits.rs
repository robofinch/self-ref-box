...
/// `for<'any> LifetimeFamily<'any, 'static>` does not automatically imply implementations
/// of `LifetimeFamily` for any other combinations of lifetimes, even though, semantically,
/// we can reason that `for<'any> LifetimeFamily<'any, 'static>` applies maximally loose bounds
/// on `'varying` and should allow for upper bounds shorter than `'static`.
///
/// You should ensure that of your implementation can use weaker lifetime bounds. In particular,
/// provide the strongest guarantees you can (as many lifetime values as possible) and make
/// use weakest bounds you can (as few lifetime values as possible) for `LifetimeFamily`.
...
/// means (such as [`transmute`]), you must call the [`CovariantFamily::covariant_assertions`]
/// method. The other two methods are not guaranteed to internally call `covariant_assertions`.
///
...
/// # Examples
///
/// If the compiler considers `'varying` to be covariant in the lifetime family, then this trait
...
/// `&'a [u8]`.
///
/// Additionally, `'varying` might have some non-covariant variance in the family assigned by the
...
/// means (such as [`transmute`]), you must call the
/// [`ContravariantFamily::contravariant_assertions`] method. The other two methods are not
/// guaranteed to internally call `contravariant_assertions`.
///
/// # Implementation Safety
/// The following three conditions must be met.
///
...
/// ## Examples
///
/// If the compiler considers `'varying` to be contravariant in the lifetime family, then this
...
/// `&'a [u8]`.
///
/// Additionally, `'varying` might have some non-contravariant variance in the family assigned by
